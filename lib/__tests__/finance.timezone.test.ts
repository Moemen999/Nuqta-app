...
describe(`حسابات التواريخ بتوقيت ${process.env.TZ || '(توقيت الجهاز)'}`, () => {
  ...
  it('السلوك الحالي: يوم 31 + شهر بيعدّي الشهر القصير كله (محتاج قرار منتج)', () => {
    // 31 يناير + شهر = 3 مارس، مش 28 فبراير — يعني اشتراك مستحق يوم 31 بيفوّت
    // فبراير بالكامل، والرجوع خطوة ورا مش بيرجّعه لمكانه.
    // ده سلوك جافاسكريبت الطبيعي وسايبينه زي ما هو دلوقتي عشان الإصلاح ده يكون
    // عن التوقيت بس. الاختبار هنا بيوثّقه عشان ميعديش من غير ما حد ياخد باله.
    expect(addMonths('2026-01-31', 1)).toBe('2026-03-03');
    expect(addMonths(addMonths('2026-01-31', 1), -1)).toBe('2026-02-03');
    expect(addMonths('2026-03-31', 1)).toBe('2026-05-01');
  });
