...
- **سجل لسه بيرفع بيبان "مش موجود" للعملية الذرية** — نتيجة مباشرة للي فوق: المستخدم يعمل اشتراك ويدوس "سدّد" قبل ما الاشتراك يوصل السيرفر، فـ `runTransaction` بتقرا من السيرفر وتلاقيه مش موجود وتخرج من غير ما تعمل حاجة (الزرار يشتغل ومفيش سداد). الحل `waitForPendingWrites(db)` قبل أي عملية ذرية
