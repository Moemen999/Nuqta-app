...
3. **التقارير (Reports)** — رسم دائري واحد للمصروفات حسب الفئة (`buildPieSlices`) مع فلتر فترة وفلتر فئات، ونسبة التغيّر عن الفترة اللي فاتت. **مفيش رسم اتجاه شهري.**
4. **التخطيط (Planning)**:
...
6. **قواعد Firestore بتتحقق من الحقول المطلوبة بس — مش من شكل المستند كله.**
   `firestore.rules` بتتأكد إن الحقول المطلوبة موجودة وأنواعها صح،
   و**مبتمنعش أي حقل زيادة**. الدين مثلًا بيتفحص فيه `personName`
   و`totalAmount` و`direction` وبس — عشان كده `dueDate` و`reminderDaysBefore`
   اتشحنوا من غير ما حد يلمس القواعد. (لو لقيت مكتوب في أي مكان إن أي حقل
   جديد لازم يتضاف في القواعد وإلا العملية هتفشل — ده كلام غلط اتصلح.)

   والنتيجة العملية: **مفيش حماية من جهة السيرفر على أي حقل مش مذكور في
   القواعد**. فأي تحقق على قيمة (مبلغ موجب، نسبة من 0 لـ100، نص مش فاضي)
   لازم يتكتب **في الكلاينت** كمان.
...
- **`silent-failure-hunter` جولتين بالظبط لكل تغيير** (2026-09-25): مرة بعد
  الكود، ومرة بعد ما **كل** التصليحات تتطبق — مش جولة تالتة. التانية
  بتتأكد إن التصليحات نفسها ما بلعتش حاجة، ولو لقت حاجة بتتصلح وتتقال في
  رسالة الكوميت من غير جولة كمان. (بند 6 خد ٣ جولات، والتالتة ملقتش جديد
  يستاهل تمنها.) ده **مش** بيلغي الإجباري: `money-reviewer` +
  `silent-failure-hunter` لسه على كل تغيير بيلمس فلوس.
...
## Git — ممنوع إعادة كتابة تاريخ اتعمله push

(2026-09-25) **مفيش force-push على أي فرع من غير ما مؤمن يوافق الأول —
وعلى `main` ممنوع خالص، حتى بموافقة.** ده بيشمل `--force` و`--force-with-lease`
وأي حاجة بتوصل لنفس النتيجة: `commit --amend` أو `rebase` أو `reset` على
كوميت اتعمله push وبعدين push تاني.

**الكوميت اللي اتعمله push محتاج تصليح؟ كوميت جديد فوقه**، مش تعديل عليه.
قاعدة "كوميت لكل بند" معناها البند ميتخلطش مع بند تاني — مش إن تصليح
المراجعة لازم يتدمج في نفس الكوميت. التاريخ اللي فيه "الكود" وبعده
"تصليحات المراجعة" أوضح وأأمن من كوميت اتكتب فوقه.

قبل الـpush، الكوميتات المحلية مسموح تتعدّل (`--amend`، `--fixup` +
`rebase --autosquash`) — لسه محدش شافها.

**ليه:** يوم 2026-09-25 كوميت أرشفة الاشتراك (`d5d61e8`) اتعدّل بعد الـpush
عشان تصليحات المراجعة تبقى جواه، واتعمل `--force-with-lease` فبقى
`8f55a79`. مكانش لازم — كوميت جديد كان هيوصل لنفس الكود. والـforce-push
بيمسح كوميت من الفرع البعيد: أي حد (جهاز تاني، Antigravity، جلسة سحابية)
كان بنى عليه بيلاقي تاريخه اتغيّر من تحته.
...
وفاكر إن Claude Code و Antigravity مبيشتغلوش على الريبو في نفس الوقت — وده
بقى بقفل (`.agent-lock`) مش بالذاكرة (شوف القسمين اللي تحت).

## Antigravity — بحث + كود مش مالي على فرعه (`GEMINI.md`)

فيه ملف `GEMINI.md` في جذر الريبو بيقول لـ Google Antigravity يعمل إيه هنا.
**اتغيّر 2026-09-25:** كان قراية وتقارير بس، ودلوقتي **مسموح يكتب كود** —
بشروط:

- **على فرعه بس:** `antigravity/<الموضوع>`، مش `main` ولا `claude/*`.
- **برّه مسارات الفلوس:** الأرصدة، الديون، الدفعات، الدخل، الأرشفة، التسوية،
  `firestore.rules`، وأي حاجة في نطاق `money-reviewer`. `GEMINI.md` فيه
  قايمة الملفات بالاسم؛ لو شغله وصل لواحد منهم بيوقف ويكتب تقرير.
- **مبيبنيش، ومبيعملش push على `main`، ومبيدمجش** — ولا حتى فرعه.
- **ومبيعدّلش** `CLAUDE.md` ولا `TIMELINE.md` ولا `GEMINI.md`، ولا
  `app.config.js`/`eas.json`/`.github/` (عدّاد البناء)، ولا الاعتماديات.
- التقارير لسه في `reports/`.

**كوده معلومة لحد ما يتراجع — بالظبط زي تقاريره.** Claude Code هو اللي
بيراجع فرع `antigravity/*` وهو اللي بيدمجه، وبنفس جدول الوكلاء (نصوص →
`arabic-copy-reviewer`، شاشة → `visual-identity-reviewer`...). كونه اتعمله
كوميت مش معناه إنه اتقبل. ولو الفرق لمس ملف في الممنوع، الفرع كله بيرجع
مش بيتدمج جزء منه.

**وده لسه كلام مكتوب في ملف، مش قفل متنفّذ.** مفيش حاجة في الريبو بتمنع
Antigravity فعليًا من تعديل أي ملف. فقبل دمج أي فرع منه:
`git diff --name-only main...antigravity/<الموضوع>` وبص على كل مسار. وبعد أي
جلسة ليه على نسخة محلية: `git status`، وأي تعديل على `main` أو برّه فرعه
يترفض ويترجع (`git checkout -- <file>`) مهما كان شكله منطقي.

## قفل الأدوات — `.agent-lock`

أداة واحدة بس (Claude Code أو Antigravity) شغّالة على نسخة الريبو في نفس
الوقت. السكريبت `scripts/agent-lock.sh`:

```
scripts/agent-lock.sh acquire claude-code    # أو antigravity — قبل أي شغل
scripts/agent-lock.sh release claude-code    # أول ما تخلص
scripts/agent-lock.sh status                 # مين شايله ومن امتى
```

- الملف `.agent-lock` في جذر الريبو، فيه `tool=` و`started=` (UTC) و`branch=`.
- `acquire` **بيفشل لو فيه قفل** — ساعتها متبدأش. بيتكتب بـ`noclobber`
  فلو أداتين حاولوا في نفس اللحظة واحدة بس تكسب.
- `release` بيشيل القفل **بتاعك بس** — مش بيشيل قفل أداة تانية.
- الملف في `.gitignore`: **القفل محلي للنسخة دي.** بيمنع أداتين على نفس
  الفولدر (Claude Code على الجهاز + Antigravity). **مبيشوفش** جلسة Claude
  Code سحابية — دي بتشتغل على نسخة متكلونة لوحدها. اللي بيحمي في الحالة دي
  إن كل أداة على فرعها والدمج بيعدّي على Claude Code.

**قفل قديم (الأداة وقعت ومشالتوش):** `status` بيحذّر لو القفل أقدم من ١٢
ساعة. **محدش بيشيله أوتوماتيك** — مؤمن بس:
1. اتأكد إن الأداة اللي في `tool=` مش شغّالة فعلاً (الشباك مقفول، مفيش
   جلسة مفتوحة).
2. `git status` — لو فيه تعديلات، دي شغل الأداة اللي وقعت: بصلها قبل أي
   حاجة (كوميت على فرعها أو `git stash`)، متمسحهاش.
3. `rm .agent-lock`
...
- **الكيبورد بيغطي حقول الإدخال** — دي قاعدة 5 فوق، والتفاصيل وحارس الاختبار في قسم "زرار الحفظ تحت الكيبورد" تحت
...
- `app/(auth)/index.tsx` — فيها `KeyboardAvoidingView` و`ScrollView` فعلاً
  والفورم قصير
...
`security.emulator.test.tsx` بس (دي بتختبر القواعد نفسها ومش
...
**اتعدّل 2026-09-25:** كاش محلي لأول فتحة باردة (آخر أرصدة وعمليات معروفة)
اتقبل ومستني جدولة — شوف `TIMELINE.md`. ده مش هجرة، وقرار "مفيش RNFB" لسه قايم.
